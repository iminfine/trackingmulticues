...
from pytracking.libs import globalmovment as gbmove
from pytracking.libs import kalmanfilter
import torch
...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        try:
            tracker.params.use_motion
        except AttributeError:
            tracker.params.use_motion=False
        
        if tracker.params.use_motion:                                  
            globalmove=gbmove.globalmotion(image,tracker.params.use_gpu,tracker.params.visualizationSEKD)
            klfilter=kalmanfilter.motionfilter(init_info.get('init_bbox'))
        ...
        self.prevboxes=[]
        self.prevboxes.append(init_info.get('init_bbox'))
        self.delta=[]
        self.outputflag=[]
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            pbox=[]
            if tracker.params.use_motion:
                delta=globalmove(image)
                klfilter.transfer(delta)
                pbox,vobject=klfilter.predict()
                self.delta.append(delta)
            out = tracker.track(image, vobject,pbox,info)
            if tracker.params.use_motion:
                if out['flag']!='not_found' and out['kfupdate_flag']:
                    klfilter.update(out['target_bbox'])
                if frame_num % tracker.params.duration==0:
                    klfilter.reset(out['target_bbox'],vobject)
                if all(self.outputflag[-tracker.params.checkoutput:]) or out['reset_flag']:
                    klfilter.reset(out['target_bbox'],(0,0))
            self.prevboxes.append(out['target_bbox'])
            self.outputflag.append(out['flag']=='not_found')
            prev_output = OrderedDict(out)
            ...
            if self.visdom is not None:
                if tracker.params.use_motion:
                    boxes={}
                    boxes['matchingbox']=out['target_bbox']
                    boxes['motionbox']=pbox
                    tracker.visdom_draw_tracking(image, pbox, segmentation)
                else:
                ...
                if tracker.params.use_motion:
                    ...
                    self.visualize(image, boxes, segmentation)
                else:
    ...
    def visualize(self, image, state, segmentation=None):
        ...
        '''
        plt.savefig('final'+str(self.frame_num)+'.tiff',dpi=300,bbox_inches='tight')
        plt.pause(0.0001)
        '''
