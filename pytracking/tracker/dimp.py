...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.helf_search=math.sqrt(search_area)/2
        self.lastv=[0,0]
        self.prev=[]
        self.lastloc=[]
        self.kfupdate_flag=True
        self.reset_flag=False
        return out


    def track(self, image, v,motionbox,info: dict = None) -> dict:
        self.debug_info = {}
        ...
        # Convert image
        im = numpy_to_torch(image)
        if self.params.use_motion:
            # ------- LOCALIZATION ------- #
            if np.abs(v).max()>0.35*self.helf_search:
                # Reset search area
                search_area = torch.prod(self.target_sz * (self.params.search_area_scale+1)).item()
                self.target_scale =  math.sqrt(search_area) / self.img_sample_sz.prod().sqrt()
                # Target size in base scale
                self.base_target_sz = self.target_sz / self.target_scale
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        self.helf_search=0.5*(sample_coords[0][2]-sample_coords[0][0]).item()
        # Extract classification features
        test_x = self.get_classification_features(backbone_feat)

        # Location of sample
        sample_pos, sample_scales = self.get_sample_location(sample_coords)
        image=image[sample_coords[0,0]:sample_coords[0,2],sample_coords[0,1]:sample_coords[0,3]]


        # Compute classification scores
        scores_raw = self.classify_target(test_x)
        '''
        score_np=scores_raw.squeeze().cpu().numpy()
        a=sample_coords[0,2]-sample_coords[0,0]
        score_np=cv2.resize(score_np,(a,a))
        fig2 = plt.figure(3)
        fig2.clf()
        ax1 = fig2.add_subplot(1, 1, 1)
        ax1.imshow(score_np,'jet')
        ax1.set_axis_off()
        plt.savefig('score'+str(self.frame_num)+'.tiff',dpi=300,bbox_inches='tight')
        plt.pause(0.0001)
        '''
        if self.params.use_motion:
            dt=1   
            a=(v-self.lastv)/dt
            s=v*dt+a*dt**2/2
            s=s/(sample_scales[0].cpu().numpy())
            s_inscore=s/16
            self.lastv=v
            self.prev.append(v)
            diff1=np.round(((self.get_centered_sample_pos()-sample_pos)/(16*sample_scales)).cpu().numpy())[0].astype(np.int32)
            helf_size=int(test_x.shape[-1]/2)
            sample_pos_inscore=helf_size+diff1
            box=self.imcrop(scores_raw,(sample_pos_inscore[1],sample_pos_inscore[0]),(5,5),False)
            xrange=[box[1],box[1]+5]
            yrange=[box[0],box[0]+5]
            
            pbox=torch.tensor(motionbox,device=self.pos.device)
            ploc=pbox[[1,0]]+pbox[[3,2]]/2
            diff2=np.round(((self.get_centered_mothon_pos(ploc)-sample_pos)/(16*sample_scales)).cpu().numpy())[0].astype(np.int32)
            motionloc_inscore=helf_size+diff2
            self.reset_flag=self.checkpos(motionloc_inscore,scores_raw)
            score_np=scores_raw.squeeze().cpu().numpy()
            
            if self.params.visualizationmotionguide:
                fig2 = plt.figure(2)
                fig2.clf()
                ax1 = fig2.add_subplot(1, 2, 1)
                ax1.imshow(scores_raw.squeeze().cpu().numpy(),'jet')
                rect = patches.Rectangle((box[0]-1,box[1]-1),box[2],box[3],linewidth=1,edgecolor='g',facecolor='none')
                ax1.add_patch(rect)
                ax1.arrow(sample_pos_inscore[1],sample_pos_inscore[0],s_inscore[0],s_inscore[1],width=0.5,color='g')
                ax1.text(2,2,str(self.frame_num))
                plt.pause(0.00001)
                
            target_patch=score_np[xrange[0]:xrange[1],yrange[0]:yrange[1]]
            thres=np.min(score_np)+self.params.motiondistractor_threshold*(np.max(score_np)-np.min(score_np))
            if self.params.centermax:
                thres=np.min(score_np)+self.params.motiondistractor_threshold*(np.max(target_patch)-np.min(score_np))
            locs=peak_local_max(score_np,min_distance=1,threshold_abs=thres)
            angles=[]
            notmove=[]
            if len(locs)>1:
                #self.kfupdate_flag=False
                for i in self.prev[-5:]:
                    angles.append(self.get_angle(i))
                    if np.abs(i).max()>2:
                        notmove.append(False)
                    else:
                        notmove.append(True)
                        
                condition=np.array(self.transformangles(angles)).std()<10        
                if self.params.checkmotion:
                    condition=all(notmove) or np.array(angles).std()<10
                if condition and (not self.reset_flag):
                    motion_probability=self.gaussian(motionloc_inscore,scores_raw)
                    maxbefore=scores_raw.max()
                    if maxbefore>self.params.target_not_found_threshold:
                        scores_raw=torch.mul(scores_raw,motion_probability)
                        maxafter=scores_raw.max()
                        scores_raw=scores_raw*(maxbefore/maxafter)
                        if self.params.visualizationmotionguide:
                            ax2 = fig2.add_subplot(1, 2, 2)
                            ax2.imshow(scores_raw.squeeze().cpu().numpy(),'jet')
                            plt.pause(0.00001)
        ...
        out = {'target_bbox': output_state,'flag':flag,'kfupdate_flag':self.kfupdate_flag,'reset_flag':self.reset_flag}
        return out
    
    def get_angle(self,v):
        """Get the angle of this velocity with the horizontal axis."""
    ...
    def checkpos(self,pos,score):
        out=True
        w=score.shape[-1]
        if pos[0]>0 and pos[0]<w and pos[1]>0 and pos[1]<w:
            out=False
        return out
    
    def check_dist(self,pos,score):
        score_np=score.squeeze().cpu().numpy()
        loc=np.unravel_index(score_np.argmax(), score_np.shape)
        dist=self.dist(pos,loc)
        out=True
        if dist>=2:
            out=False
        return out
            
    def gaussian(self,pos,score_raw):
        score_sz=score_raw.shape[-1]  
        sz=[self.odd(i) for i in 2*pos]
        gaussian=kornia.filters.get_gaussian_kernel2d(tuple(sz), (1, 1))
        out=gaussian[0:score_sz,0:score_sz]/gaussian.max()
        outsz=out.shape
        out=out.unsqueeze(0).unsqueeze(0).cuda()
        pad=False
        padright=0
        padbottom=0
        if outsz[1]<score_sz:
            padright=score_sz-outsz[1]
            pad=True
        if outsz[0]<score_sz:
            padbottom=score_sz-outsz[0]
            pad=True
        if pad:
            out=F.pad(out,(0,padright,0,padbottom))
        '''
        fig2 = plt.figure(3)
        fig2.clf()
        ax1 = fig2.add_subplot(1, 1, 1)
        ax1.imshow(out.squeeze().cpu().numpy(),'jet')
        ax1.set_axis_off()
        plt.pause(0.0001)
        '''
        return out
    ...
    def imcrop(self,I,center,size,outpatch=True):
        b,c,w,h=I.shape
        box=(int(center[0])-(size[0]-1)/2,int(center[1])-(size[1]-1)/2,size[0],size[1])
        boxInbounds=(min(h-self.odd(box[2]),max(0,round(box[0]))),min(w-self.odd(box[3]),max(0,round(box[1]))),
                     self.odd(box[2]),self.odd(box[3]))
        box=boxInbounds
        if outpatch:
    ...
    def get_centered_mothon_pos(self,pos):
